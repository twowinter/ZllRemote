/**************************************************************************************************
  ...
  Revised:        $Date: 2013-10-24 16:11:31 -0700 (Thu, 24 Oct 2013) $
  Revision:       $Revision: 35793 $

  Description:    Declarations for the CC2530 ZLL RC
 ...
 *                                       Board Indentifier
 *
 *      Define the Board Identifier to CC2530 ZLL RC board
 * ------------------------------------------------------------------------------------------------
 */
#define HAL_BOARD_CC2530ZLLRC
...
//LED 3 and 4 used by ZDApp. We do not want LED's to be driven by anyting other
//than App.
...
// dummy call for assert
#define HAL_PUSH_BUTTON1() 1
...
{                                                                \
  ...
  LED6_DDR |= LED6_BV;                                           \
                                                                 \
  /* set I/O ports to minimize power drainage */                 \
  P2DIR |= 0x08;                              /* P2.3 set as output (control DC/DC) */            \
  P1DIR = 0xF1;                               /* P1.0, P1.4-P1.7 set as output (SPI module disabled) */  \
  P1INP |= 0x0E;                              /* P1.1-P1.3 (LEDs) set as tri-state */             \
  P2DIR &= ~(0x01);                           /* P2.0 (LED) set as input */                   \
  P2INP |= 0x01;                              /* P2.0 set as tri-state */                         \
  P1_0 = 0; \
  P1_4 = 0; \
  P1_5 = 0; \
  P1_6 = 0; \
...
//Can not reduce threshold to take into account a 5% margin as the DCDC is supplying 2.08v
//#define VDD_MIN_NV   (VDD_2_0+4)  // 5% margin over minimum to survive a page erase and compaction.
#define VDD_MIN_NV   (VDD_2_0+2)  // minimal margin over minimum to survive a page erase and compaction.
